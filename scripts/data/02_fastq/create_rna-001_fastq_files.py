...
if __name__ == "__main__":
    ...
    # Step 3. Fetch TP53 sequence and create a somatic SNV at position 7675118 (T>C)
    ...
    for _,row in df_exons_tp53.iterrows():
        ...
        if exon_number == 5:
            # Create a SNV at position 7675118
            reference_position = 7675118
            local_position = len(normal_sequence) - (end - reference_position) - 1
            assert normal_sequence[local_position] == 'T'
            tumor_sequence = normal_sequence[:local_position] + 'C' + normal_sequence[local_position+1:]
        else:
    ...
    create_long_read_fastq_file(
        ...
        output_fastq_file='../../../test/data/fastq/nexus-rna-001-tumor_long_read.fastq.gz',
        num_reads=[10,10],
        read_length=20000,
        stranded=True
    )
    create_paired_end_fastq_files(
        sequences=[tp53_tumor_sequence, tp53_normal_sequence],
        output_r1_fastq_file='../../../test/data/fastq/nexus-rna-001-tumor_paired-end_read_r1.fastq.gz',
        output_r2_fastq_file = '../../../test/data/fastq/nexus-rna-001-tumor_paired-end_read_r2.fastq.gz',
        num_reads=[1000,1000],
        read_length=151,
        min_insert_size=300,
        max_insert_size=600
