...
def test_quantification_longread_rna_1():
    ...
    intermediate_dir = temp_dir + '/intermediate/test_quantification_longread_rna_1'
    work_dir = temp_dir + '/work/test_quantification_longread_rna_1'
    output_dir = temp_dir + '/outputs/test_quantification_longread_rna_1'
    ...
    run_workflow(workflow='quantification_long-read-rna.nf',
                 nextflow='nextflow',
                 workflow_args=workflow_args)


# Kallisto, Oarfish, TranSigner (FASTQ-based — nexus-rna-002-tumor)
def test_quantification_longread_rna_2():
    nextflow_config_file = get_data_path(name='nextflow/nextflow_test_docker.config')
    bam_file = get_data_path(name='bam/nexus-rna-002-tumor-long-read_minimap2_mdtagged_sorted.bam')
    bam_bai_file = get_data_path(name='bam/nexus-rna-002-tumor-long-read_minimap2_mdtagged_sorted.bam.bai')
    fastq_file = get_data_path(name='fastq/nexus-rna-002-tumor_long_read.fastq.gz')
    ...
    temp_dir = os.getcwd() + '/tmp'
    intermediate_dir = temp_dir + '/intermediate/test_quantification_longread_rna_2'
    work_dir = temp_dir + '/work/test_quantification_longread_rna_2'
    output_dir = temp_dir + '/outputs/test_quantification_longread_rna_2'
    if not os.path.exists(intermediate_dir):
        os.makedirs(intermediate_dir)
    if not os.path.exists(work_dir):
        os.makedirs(work_dir)
