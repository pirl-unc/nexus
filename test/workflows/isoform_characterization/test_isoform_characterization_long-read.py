...
# ESPRESSO + SQANTI3-GTF chaining (nexus-rna-103-tumor)
...
# IsoQuant + SQANTI3-GTF chaining (nexus-rna-103-tumor)
...
# IsoTools + Mandalorion + SQANTI3-GTF chaining (nexus-rna-103-tumor)
...
# FLAIR3 + LRAA + TALON, no SQANTI3 chaining (nexus-rna-103-tumor)
def test_isoform_characterization_long_read_4():
    nextflow_config_file = get_data_path(name='nextflow/nextflow_test_docker.config')
    bam_file = get_data_path(name='bam/nexus-rna-103-tumor-long-read_minimap2_mdtagged_sorted.bam')
    bam_bai_file = get_data_path(name='bam/nexus-rna-103-tumor-long-read_minimap2_mdtagged_sorted.bam.bai')
    fastq_file = get_data_path(name='fastq/nexus-rna-103-tumor_long_read.fastq.gz')
    reference_genome_fasta_file = get_data_path(name='fasta/GRCh38.p14.genome.chr17.fa.gz')
    reference_genes_gtf_file = get_data_path(name='gtf/gencode.v45.chr_patch_hapl_scaff.annotation.chr17.gtf.gz')
    params_yaml_file = get_data_path(name='inputs/workflows/isoform_characterization_long-read/params_4.yaml')
    temp_dir = os.getcwd() + '/tmp'
    intermediate_dir = temp_dir + '/intermediate/test_isoform_characterization_longread_4'
    work_dir = temp_dir + '/work/test_isoform_characterization_longread_4'
    output_dir = temp_dir + '/outputs/test_isoform_characterization_longread_4'
    if not os.path.exists(intermediate_dir):
        os.makedirs(intermediate_dir)
    if not os.path.exists(work_dir):
        os.makedirs(work_dir)
