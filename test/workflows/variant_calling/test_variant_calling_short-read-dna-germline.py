...
def test_variant_calling_short_read_dna_germline_1():
    ...
    params_yaml_file = get_data_path(name='inputs/workflows/variant_calling_short-read-dna-germline/params_1.yaml')
    ...
    run_workflow(workflow='variant_calling_short-read-dna-germline.nf',
                 nextflow='nextflow',
                 workflow_args=workflow_args)


# Dysgu, GRIDSS2, Lumpy (SV callers — nexus-dna-002-tumor)
def test_variant_calling_short_read_dna_germline_2():
    nextflow_config_file = get_data_path(name='nextflow/nextflow_test_docker.config')
    ...
    reference_genome_fasta_file = get_data_path(name='fasta/GRCh38.p14.genome.chr17.fa.gz')
    params_yaml_file = get_data_path(name='inputs/workflows/variant_calling_short-read-dna-germline/params_2.yaml')
    temp_dir = os.getcwd() + '/tmp'
...
def test_variant_calling_short_read_dna_germline_github_3():
    nextflow_config_file = get_data_path(name='nextflow/nextflow_test_docker.config')
    bam_file = get_data_path(name='bam/nexus-dna-001-tumor-paired-end-read_fixmate_markeddup_recalibrated.bam')
    bam_bai_file = get_data_path(name='bam/nexus-dna-001-tumor-paired-end-read_fixmate_markeddup_recalibrated.bam.bai')
    reference_genome_fasta_file = get_data_path(name='fasta/GRCh38.p14.genome.chr17.fa.gz')
    params_yaml_file = get_data_path(name='inputs/workflows/variant_calling_short-read-dna-germline/params_3.yaml')
...
# DeepVariant + Octopus (nexus-dna-001-tumor)
def test_variant_calling_short_read_dna_germline_local_3():
    nextflow_config_file = get_data_path(name='nextflow/nextflow_test_docker.config')
    bam_file = get_data_path(name='bam/nexus-dna-001-tumor-paired-end-read_fixmate_markeddup_recalibrated.bam')
    bam_bai_file = get_data_path(name='bam/nexus-dna-001-tumor-paired-end-read_fixmate_markeddup_recalibrated.bam.bai')
    reference_genome_fasta_file = get_data_path(name='fasta/GRCh38.p14.genome.chr17.fa.gz')
