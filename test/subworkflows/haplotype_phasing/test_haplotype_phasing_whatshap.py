...
def test_haplotype_phasing_whatshap():
    ...
    intermediate_dir = temp_dir + '/intermediate/test_haplotype_phasing_whatshap'
    work_dir = temp_dir + '/work/test_haplotype_phasing_whatshap'
    output_dir = temp_dir + '/outputs/test_haplotype_phasing_whatshap'
    ...
    run_workflow(workflow='haplotype_phasing_whatshap.nf',
                 nextflow='nextflow',
                 workflow_args=workflow_args)

    # Verify both outputs were produced
    sample_out_dir = os.path.join(output_dir, 'nexus-dna-001-tumor')
    phased_vcf = os.path.join(sample_out_dir, 'nexus-dna-001-tumor_whatshap_phased.vcf.gz')
    phased_vcf_tbi = os.path.join(sample_out_dir, 'nexus-dna-001-tumor_whatshap_phased.vcf.gz.tbi')
