...
def test_haplotagging_longphase():
    ...
    intermediate_dir = temp_dir + '/intermediate/test_haplotagging_longphase'
    work_dir = temp_dir + '/work/test_haplotagging_longphase'
    output_dir = temp_dir + '/outputs/test_haplotagging_longphase'
    ...
    workflow_args = [
        ...
        '--params_longphase_phase', '"--pb "',
        '--params_longphase_haplotag', '""',
    ]
    run_workflow(workflow='haplotagging_longphase.nf',
                 nextflow='nextflow',
                 workflow_args=workflow_args)

    # Verify both outputs were produced.
    sample_out_dir = os.path.join(output_dir, 'nexus-dna-001-tumor')
    phased_vcf = os.path.join(sample_out_dir, 'nexus-dna-001-tumor_longphase_phased.vcf.gz')
    phased_vcf_tbi = os.path.join(sample_out_dir, 'nexus-dna-001-tumor_longphase_phased.vcf.gz.tbi')
