...
// ------------------------------------------------------------
include { runHLAminerShortReadDNA }   from '../../../tools/hlaminer'
...
// ------------------------------------------------------------
workflow HLA_TYPING_HLAMINER_SR_DNA {
    take:
...
workflow {
    log.info """\
             ...
             (bwa alignment against HLAminer's genomic HLA reference)
             ==================================================================
             """.stripIndent()
    ...
    if (params.help) {
        log.info"""\
        workflow:
            1. Align paired short-read DNA FASTQ files against HLAminer's genomic
               HLA reference (HLA-I_II_GEN.fasta) using bwa aln/sampe.
            2. Run HLAminer.pl in file mode on the paired SAM to predict HLA
               class I and II alleles.

        usage: nexus run --nf-workflow hla_typing_hlaminer-sr-dna.nf [required] [optional] [--help]
        ...
        optional arguments:
            ...
            --params_hlaminer       :   HLAminer.pl extra CLI parameters
                                        ...
                                        (HPRAwgs_classI-II.sh); short reads keep the
