## antigen_prediction_netmhcpan4.nf
...
`--netmhcpan_home_dir`
* NetMHCpan home directory path. Download NetMHCpan from https://services.healthtech.dtu.dk/services/NetMHCpan-4.1/

`--netmhcpan`
* `netMHCpan` executable path. Download NetMHCpan from https://services.healthtech.dtu.dk/services/NetMHCpan-4.1/
...
After you run this workflow, you can use `nexus_convert_netmhcpan_txt2tsv` (`nexus` built-in executable) to convert 
the `NetMHCpan4.x` output `txt` file to a `tsv` file:

```
nexus_convert_netmhcpan_txt2tsv [-h] 
  [-i INPUT_TXT_FILE]
  [-o OUTPUT_TSV_FILE]
```
