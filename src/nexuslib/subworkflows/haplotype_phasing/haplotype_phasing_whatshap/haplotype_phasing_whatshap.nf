...
include { runWhatshapPhase }                       from '../../../tools/whatshap'
...
// Optional arguments
params.params_whatshap_phase            = '--mapq 20'
params.params_whatshap_haplotag         = '--ignore-read-groups --tag-supplementary --skip-missing-contigs --output-threads 4'

// ------------------------------------------------------------
// Step 3. Sub-workflows
// ------------------------------------------------------------
workflow HAPLOTYPE_PHASING_WHATSHAP {
    take:
        input_bam_vcf_files_ch             // channel: [val(sample_id), path(bam_file), path(bam_bai_file), path(small_variants_vcf_file)]
        reference_genome_fasta_file
        params_whatshap_phase
    ...
    main:
        ...
        // Step 2. Run Whatshap 'phase' to produce a phased VCF
        runWhatshapPhase(
        ...
        //         producing the input expected by runWhatshapHaplotag:
        ...
        haplotag_input_ch = bam_only_ch
            .join(runWhatshapPhase.out.f, by: 0)
    ...
    emit:
        phased_vcf      = runWhatshapPhase.out.f         // [sample_id, vcf.gz, vcf.gz.tbi]
...
workflow {
    log.info """\
             ===============================================================================================
             Phase small variants and haplotag long-read DNA sequencing BAM files using Whatshap
             ===============================================================================================
             """.stripIndent()
    ...
    if (params.help) {
        log.info"""\
        workflow:
            1. Run Whatshap 'phase' command to produce a phased VCF.
            2. Run Whatshap 'haplotag' command to produce a haplotagged BAM (using the phased VCF from step 1).

        usage: nexus run --nf-workflow haplotype_phasing_whatshap.nf [required] [optional] [--help]
        ...
        optional arguments:
            --params_whatshap_phase             :   Whatshap 'phase' parameters (default: '"--mapq 20"').
                                                    Note that the parameters need to be wrapped in quotes.
            --params_whatshap_haplotag          :   Whatshap 'haplotag' parameters (default:
                                                    '"--ignore-read-groups --tag-supplementary --skip-missing-contigs --output-threads 4"').
                                                    Note that the parameters need to be wrapped in quotes.
        """.stripIndent()
        exit 0
    }

    def params_whatshap_phase    = (params.params_whatshap_phase    == true) ? '' : params.params_whatshap_phase
    ...
    log.info"""\
        ...
        params_whatshap_phase               :   ${params_whatshap_phase}
