...
process runDeepVariantSingularity {
    ...
    publishDir(
        path: "${output_dir}/${sample_id}/deepvariant/",
        mode: 'copy'
    )
    ...
    output:
        tuple val(sample_id), path("${sample_id}_deepvariant.vcf.gz"), path("${sample_id}_deepvariant.gvcf.gz"), emit: f
...
process runDeepVariantDocker {
    ...
    script:
        ...
        docker run \
            ...
            --ref=$reference_genome_fasta_file \
            --reads=$bam_file \
            --output_vcf=\${PWD}/${sample_id}_deepvariant.vcf.gz \
            --output_gvcf=\${PWD}/${sample_id}_deepvariant.gvcf.gz \
            --intermediate_results_dir=\${PWD}/${sample_id}_deepvariant_intermediate_results/ \
            --num_shards=${task.cpus}
        """
}