...
process runGatk4CreateSequenceDictionary {

    label 'gatk4_createsequencedictionary'
    debug true
    ...
    output:
        path("${dict_name}"), emit: f

    script:
        dict_name = fasta_file.name.replaceAll(/\.(fa|fasta|fas)(\.gz)?$/, '.dict')
        """
        gatk --java-options -Xmx${task.java_max_mem.toGiga()}G CreateSequenceDictionary \
            -R $fasta_file \
            -O ${dict_name}
        """
}

process runGatk4HaplotypeCaller {
    ...
    publishDir(
        path: "${output_dir}/${sample_id}/haplotypecaller/",
        mode: 'copy'
    )
    ...
    input:
        ...
        val(params_gatk4haplotypecaller)
        val(output_dir)

    output:
        tuple val(sample_id), path("${sample_id}_gatk4-haplotypecaller_${chromosome}.vcf.gz"), emit: f

    script:
        """
        gatk --java-options -Xmx${task.java_max_mem.toGiga()}G HaplotypeCaller \
            -R $reference_genome_fasta_file \
            -I $bam_file \
            --intervals $chromosome \
            -O ${sample_id}_gatk4-haplotypecaller_${chromosome}.vcf.gz \
            $params_gatk4haplotypecaller
        """
}

process runGatk4IndexFeature {

    label 'gatk4_indexfeature'
    debug true
    ...
    output:
        path("*.idx"), optional: true, emit: idx
...
process runGatk4BaseRecalibrator {
    ...
    input:
        ...
        path(reference_genome_fasta_gzi_file)
        path(reference_genome_fasta_dict_file)
        path(known_sites_vcf_files)
        path(known_sites_vcf_idx_files)

    output:
         tuple val(sample_id), path(bam_file), val(chromosome), path("${sample_id}_recalibration_${chromosome}.data.table"), emit: f

    script:
        known_sites_args = known_sites_vcf_files.collect { "--known-sites $it" }.join(' ')
        """
...
process runGatk4GetPileupSummaries {
    ...
    input:
        ...
        path(variant_vcf_file)
        path(variant_vcf_index_file)
...
process runGatk4Mutect2TumorNormal {
    ...
    input:
        ...
        path(reference_genome_fasta_gzi_file)
        path(reference_genome_fasta_dict_file)
        path(germline_resource_vcf_file)
        path(germline_resource_vcf_index_file)
        path(panel_of_normals_vcf_file)
        path(panel_of_normals_vcf_index_file)
    ...
    script:
        def has_germline = (
            germline_resource_vcf_file.name != 'NO_GERMLINE_FILE' &&
            germline_resource_vcf_file.exists() &&
            germline_resource_vcf_index_file.name != 'NO_GERMLINE_INDEX_FILE' &&
            germline_resource_vcf_index_file.exists()
        )
        def germline_opt = has_germline ? "--germline-resource ${germline_resource_vcf_file}" : ""

        def has_panel_of_normals = (
            panel_of_normals_vcf_file.name != 'NO_PON_FILE' &&
            panel_of_normals_vcf_file.exists() &&
            panel_of_normals_vcf_index_file.name != 'NO_PON_INDEX_FILE' &&
            panel_of_normals_vcf_index_file.exists()
        )
        def panel_of_normals_opt = has_panel_of_normals ? "--panel-of-normals ${panel_of_normals_vcf_file}" : ""
        """
