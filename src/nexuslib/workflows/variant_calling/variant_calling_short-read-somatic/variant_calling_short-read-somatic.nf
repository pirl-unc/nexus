...
if (params.help) {
    log.info """\
    usage: nexus run --nf-workflow variant_calling_short-read-somatic.nf -params-file params.yaml [--help]
...
Channel
    ...
    .map { row -> tuple(
        ...
        "${row.tumor_bam_file_no_abra2}",
        "${row.tumor_bam_bai_file_no_abra2}",
        "${row.normal_bam_file_no_abra2}",
        "${row.normal_bam_bai_file_no_abra2}",
...
workflow VARIANT_CALLING_SHORTREAD_SOMATIC {
    ...
    main:
        ...
        no_abra2_bam_files_ch = input_bam_files_ch.map { it ->
