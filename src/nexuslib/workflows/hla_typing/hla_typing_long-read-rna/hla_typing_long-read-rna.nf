...
if (params.help) {
    log.info """\
    usage: nexus run --nf-workflow hla_typing_long-read-rna.nf -params-file params.yaml [--help]
...
def run_hlaminer_lr_rna  = run_all || active_methods.contains('hlaminer')
...
// Step 4. Set channels
// ------------------------------------------------------------
// FASTQ channel — single long-read RNA FASTQ per sample
// (used by HLAminer (lr-rna) and SpecImmune)
Channel
...
// ------------------------------------------------------------
workflow HLA_TYPING_LONGREAD_RNA {
    take:
    ...
    main:
        if (run_hlaminer_lr_rna) {
            HLA_TYPING_HLAMINER_LR_RNA(
                ...
                cfg_hlaminer.hlaminer_extra_args ?: '-e 1 -s 500 -q 1 -i 1',
                output_dir
            )
        }
