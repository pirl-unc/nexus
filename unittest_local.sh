...
# Exclude test_paired_end_read_dna_somatic_small_variants_human_docker because strelka2 only run on linux
# Exclude test_paired_end_read_dna_somatic_small_variants_human_singularity because strelka2 only run on linux
pytest \
  -s \
