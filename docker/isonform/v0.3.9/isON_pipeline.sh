...
# Every "Missing parameter" branch below calls die, but it was never defined:
# under `set -e` the branches aborted with "die: command not found" and exit 127,
# swallowing the message they meant to print. Define it once here so all of them
# report the actual missing parameter.
function die {
    echo "$@" >&2
    exit 1
}
...
# iso_abundance is required in practice but was never validated: it is
# interpolated unquoted into `isONclust write_fastq --N $iso_abundance` below,
# so omitting it silently emits `--N --clusters ...` and argparse dies 200 lines
# into the log with "argument --N: expected one argument" (exit 2). Fail here
# instead, with the same message shape as the other required parameters. No
# default is invented: --N is also isONclust's minimum reads per cluster, so the
# value is a scientific choice for the caller, not something to guess.
